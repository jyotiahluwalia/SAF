...
def resize_and_prepare_masks(masks, img_shape):
    """
    Resize and prepare masks to match the image shape.
    
    Args:
        masks (list or np.ndarray): List of RLE-encoded masks or binary masks.
        img_shape (tuple): The target shape (height, width) to resize the masks.
    ...
    if isinstance(masks, list) and isinstance(masks[0], dict):
        ...
        decoded_masks = [coco_mask.decode(mask).astype(np.uint8) for mask in masks]
        resized_masks = [mmcv.imresize(mask, (img_shape[1], img_shape[0]), interpolation='nearest') for mask in decoded_masks]
    elif isinstance(masks, np.ndarray) and masks.ndim == 3:
        # Handle binary masks
        resized_masks = [mmcv.imresize(mask, (img_shape[1], img_shape[0]), interpolation='nearest') for mask in masks]
    else:
    ...
    def _save_image_gts_results(self,
        ...
        for performance_info in performances:
            ...
            if task == 'det':
                ...
                """Mask code"""
                if 'masks' in results[index]['pred_instances']:
                    pred_masks = results[index]['pred_instances']['masks']
                    img = mmcv.imread(filename)
                    img_shape = img.shape[:2]
                    print(f"Image shape: {img_shape}")
    
                    resized_masks = resize_and_prepare_masks(pred_masks, img_shape)
                    print(f"Resized masks shape: {resized_masks.shape}")
    
                    pred_instances.masks = resized_masks
